...
const LICENSE_DIRS = [
  { dir: 'ofl', code: 'OFL', name: 'SIL Open Font License 1.1', sample: 'abeezee', file: 'OFL.txt' },
  { dir: 'apache', code: 'APACHE2', name: 'Apache License 2.0', sample: 'aclonica', file: 'LICENSE.txt' },
  { dir: 'ufl', code: 'UFL', name: 'Ubuntu Font License 1.0', sample: 'ubuntu', file: 'UFL.txt' },
  { dir: 'cc-by-sa', code: 'CC-BY-SA', name: 'Creative Commons Attribution-ShareAlike 4.0', sample: 'knowledge', file: 'LICENSE.txt' },
];
...
function renderMarkdown(entries, licenseTexts) {
  ...
  for (const e of entries) {
    lines.push(`| ${safe(e.family)} | ${safe(e.licenseCode || 'unknown')} | ${safe(e.designer)} | ${safe(e.copyright)} |`);
  }
  lines.push('');
...
(async () => {
  ...
  if (missing.length) {
    ...
    dirSlugs = {};
    for (const lic of LICENSE_DIRS) {
      try { dirSlugs[lic.dir] = await getDirSlugs(lic.dir); }
      catch (err) {
    ...
    await runPool(missing, async (family) => {
      const slug = upstreamSlug(family);
      let matched = null;
      for (const lic of LICENSE_DIRS) {
        if (dirSlugs[lic.dir].has(slug)) { matched = lic; break; }
      }
      if (!matched) {
        cache.entries[family] = { family, slug, licenseCode: null, designer: '', copyright: '', resolved: false };
        return;
      }
  ...
  const usedCodes = new Set(Object.values(cache.entries)
  ...
  const sortedEntries = {};
  for (const family of Object.keys(cache.entries).sort()) sortedEntries[family] = cache.entries[family];
  const sortedTexts = {};
  for (const code of Object.keys(cache.licenseTexts).sort()) sortedTexts[code] = cache.licenseTexts[code];
  fs.writeFileSync(CACHE_FILE, JSON.stringify({ entries: sortedEntries, licenseTexts: sortedTexts }, null, 2) + '\n');
  ...
  console.log(`Wrote ${path.relative(ROOT, OUT_FILE)} (${rendered.length} fonts, ${unresolved.length} unresolved).`);
