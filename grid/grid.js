(function () {
  ...
  // The grid character lives in the path: /grid/<char>. A bare /grid/ (no
  // char) redirects to a random one, so every visit lands on a real char.
  const m = location.pathname.match(/^\/grid\/(.+)/);
  const ch = m ? [...decodeURIComponent(m[1].replace(/\/+$/, ''))][0] || '' : '';
  if (!ch) {
    location.replace('/grid/' + encodeURIComponent(pickChar()));
    return;
  }
  window.GRID = ch;
  document.title = ch;
  document.body.classList.add('grid-mode');
  Common.paintFavicon({ glyph: ch });
